...
const getGraphData = async (req, res) => {
  ...
  const county = String(req.query.county).toUpperCase();
  // console.log("API states:", state[state.length - 1]);
  const startDate = req.query.startDate;
  const date = new Date();
  const endDate =
    new Date(req.query.endDate).toISOString().split("T")[0] + "T" + "23:59:59Z";
  // const countyData = [];
  try {
    // for (let i = 0; state.length > i; i++) {
    const currentCountyData = await countyDataModel.getCountyGraphData(
      state,
      county,
      startDate,
      endDate
    ...
    const countyData = currentCountyData.sort((a, b) => {
